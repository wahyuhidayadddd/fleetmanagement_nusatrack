...
export default function HeaderLinks(props) {
  ...
  const handleLogout = () => {
    // Hapus token atau flag otentikasi dari localStorage
    localStorage.removeItem('token');
    localStorage.removeItem('isAuthenticated'); // Jika menggunakan kunci ini
  ...
  // const [features, setFeatures] = useState([]); // State to hold features data

  const lihatDataUser = async () => {
      const token = localStorage.getItem('token');
console.log('Token:', token); // Pastikan token tertera
    ...
    if (response.ok) {
      ...
      if (data && data.user) {
        ...
        // setFeatures(data.features); // Set features data in state
      } else {
  ...
  useEffect(() => {
    const token = localStorage.getItem('token');
    console.log('Token di useEffect:', token); // Log token di sini
    if (token) {
        lihatDataUser(); 
    } else {
        console.error('Token tidak ditemukan, pastikan pengguna telah login.');
    }
}, []);
